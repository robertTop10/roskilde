...
$lang = (isset($_GET["dn"]) && $_GET["dn"] === 'true') ? 'dn' : 'en';

$xmlNodeUK = simplexml_load_file('lineup2013-en.xml');
$arrayDataUK = xmlToArray($xmlNodeUK);

$xmlNodeDK = simplexml_load_file('lineup2013-dn.xml');
$arrayDataDK = xmlToArray($xmlNodeDK);
...
foreach ($arrayDataUK['bandPreview']['item'] as $key=>&$value) {
...
$result = (object) array("results"=>$days, "keys"=>array_keys($days), "stages"=>$stageNames, "artists"=>$artistsIDs);
